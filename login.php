...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $query = "SELECT id, nickname, password FROM accounts WHERE nickname = '$nickname'";
    $result = mysqli_query($conn, $query);
    
    if (mysqli_num_rows($result) > 0) {
        $user = mysqli_fetch_assoc($result);
        
        // Сравниваем хеш пароля (в реальной ситуации нужно использовать более безопасные методы)
        if (strtoupper(md5($password)) === $user['password']) {
...
<head>
    ...
    <title>Вход в личный кабинет - Dream Mobile</title>
    ...
    <header class="fixed w-full py-4 z-50 transition-all duration-300" id="main-header">
            ...
            <div class="flex justify-between items-center">
                ...
                <nav class="hidden md:block">
                    <ul class="flex space-x-8">
                        ...
                        <li><a href="login.php" class="bg-blue-600 bg-opacity-10 border border-blue-500 border-opacity-40 px-5 py-2 rounded-full hover:bg-opacity-20 transition-all duration-300">Личный кабинет</a></li>
                    </ul>
                </nav>
        ...
        <div class="md:hidden mobile-menu hidden bg-black bg-opacity-95 absolute w-full left-0 py-4 px-6 border-t border-gray-800 mt-4 animate-fadeIn">
            <ul class="space-y-4">
                ...
                <li><a href="login.php" class="block bg-blue-600 bg-opacity-10 border border-blue-500 border-opacity-40 px-5 py-2 rounded-full hover:bg-opacity-20 transition-all duration-300 w-full text-center mt-4">Личный кабинет</a></li>
            </ul>
        </div>
    </header>
            ...
            <div class="container mx-auto px-6">
                <div class="max-w-md mx-auto relative z-10">
                    <h1 class="text-4xl md:text-5xl font-bold font-unbounded mb-8 text-center animate-on-scroll opacity-0">
                        <span class="animate-gradient-text">Личный кабинет</span>
                    </h1>
                    <p class="text-lg mb-12 text-gray-300 text-center max-w-xl mx-auto animate-on-scroll opacity-0 delay-100">
                        Войдите в свой аккаунт, чтобы получить доступ к личной информации
                    </p>
                    ...
                    <div class="relative animate-on-scroll opacity-0 delay-200">
                        ...
                        <div class="backdrop-blur-sm bg-white/5 rounded-3xl p-8 relative z-10 border border-white/10">
                            <form action="login.php" method="post" id="login-form">
                                <div class="mb-8">
                                    <label for="nickname" class="block text-sm font-medium text-gray-300 mb-2">Ваш никнейм</label>
